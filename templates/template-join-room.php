...
<body class="dailyco-join-room">
<?php
$room = isset( $_GET['j'] ) ? $_GET['j'] : false;
if ( ! empty( $room ) ) {
	$token  = Daily_Co_Bookly_API_Helper::validating_generated_token( $room );
	...
	if ( ! empty( $token ) ) {
		if ( isset( $token['error'] ) && $token['error'] === "user-not-logged-in" ) {
			...
			if ( ! empty( $room ) && ! empty( $room->error ) ) {
				...
				wp_enqueue_script( 'daily-api-scheduler', DPEN_DAILY_CO_URI_PATH . 'assets/js/daily-api-room-scheduler.js', array(
					'jquery',
					'daily-api-script'
				), time(), true );
