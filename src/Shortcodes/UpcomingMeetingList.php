...
class UpcomingMeetingList {
	...
	public function upcomingMeetingTherapist() {
		...
		$this->current_user_id = get_current_user_id();
		$staff_detail          = Daily_Co_Bookly_Datastore::getStaffbyUserID( $this->current_user_id );
	...
	public function upcomingMeetingCustomer() {
        ...
        <table class="widefat fixed striped">
			...
			<?php
			$this->current_user_id = get_current_user_id();
			$appointments          = Daily_Co_Bookly_Datastore::get_appointments_by_customer( $this->current_user_id, '>=', 'ASC', true );
			if ( ! empty( $appointments ) ) {
				foreach ( $appointments as $appointment ) {
